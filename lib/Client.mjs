...
export default class Client {
    ...
    loadPlugin(pluginName) {
        if (!this.plugins[pluginName] || !this.plugins[pluginName].class) {
            ...
            fs.promises.access(pluginPath).then(() => {
                ...
                Log(`Plugin ${pluginName} not found!`, 1);
            });
        } else {
    ...
    initPlugin(pluginName, pluginPath) {
        ...
        import(url.href).then(plugin => {
            ...
            Log(`Plugin ${chalk.green(pluginName)} [${this.plugins[pluginName].version}] loaded`);
        }).catch(err => {
            Log(`Error loading plugin ${pluginName}: ${err}`, 1);
        });
    }
    unloadPlugin(pluginName) {
        if (this.plugins[pluginName]) {
            ...
            Log(`Plugin ${pluginName} not loaded`, 1);
        }
    }
    // /Handle plugins
    ...
    init() {
        if (this.connection.connected()) {
            Promise.all([
                ...
                this.inited = true;
                Log(`Connected to server ${this.config.defaultServer} (${this.config.auth.host}:${this.config.auth.port})`);
                this.broadcast('init');
            }).catch(err => {
                Log(`Error in init: ${JSON.stringify(err)}`, 1);
            });
        } else {
            Log("Not connected", 2);
        }
