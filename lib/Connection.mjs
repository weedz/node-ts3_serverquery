...
import { parseArgsToString, parseParams } from './Connection/Utils';
...
export default class Connection {
    ...
    connectionCallback() {
        ...
        this.pingTimer = setInterval(async () => {
            ...
            await this.send('version', undefined, {noOutput: true}, 0);
            const ping = process.hrtime(startTime);
            Log(`Ping: ${ping[0]}.${Math.round(ping[1] / 1000)}ms`, this.constructor.name, 5);
        }, 60000);
    ...
    errorHook(params) {
        if (params.id !== '0') {
            Log(`Error check failed: ${chalk.yellow(this.commandQueue.getCommand().commandStr)} ${JSON.stringify(this.commandQueue.getCommand())}, ${JSON.stringify(params)}`, this.constructor.name, 2);
    ...
    onClose(hadError) {
        ...
        Log(logMsg, this.constructor.name, logLevel);
