...
export default class Connection {
        ...
        if (this.getCommand().label === 'help') {
            ...
            for (let line of msg.split("\n")) {
                ...
                const params = this.registeredHooks[event]
                    ? Connection.parseParams(line, 1, event.length + 1)
                    : Connection.parseParams(line, SHOULD_PARSE_PARAMS[this.getCommand().label]);
    ...
    static parseParams(msg, type, start = 0) {
        if (type === 2) {
            return msg.split("|").map(item => Connection.parseParams(item, 1));
        }
        if (type !== 0) {
            const params = {};
