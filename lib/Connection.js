...
function recievedData(msg) {
    if (currentCommand.label === 'help') {
        ...
        for (let line of msg.split(/\n/g)) {
            ...
            if (typeof currentCommand.callback === 'function' && (!currentCommand.mustReturnOK || !currentCommand.failed)) {
                currentCommand.callback(params, event, line);
            }
        }
    }
}
...
function processQueue() {
    if (!currentCommand.mustReturnOK || !currentCommand.failed) {
...
function addToCommandQueue(label, options, command, callback) {
    commandQueue.push({
        label,
        command,
        callback,
