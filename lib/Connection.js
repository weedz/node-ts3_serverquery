...
class Connection {
    ...
    recievedEvent(params, event, msg) {
        if (this.registeredHooks[event]) {
            for (let callback of this.registeredHooks[event]) {
                callback(params, msg);
            }
        }
    ...
    init(rl) {
        this.connection.on('data', data => {
            ...
            const msg = data.toString('utf8').replace(/\r/g, "");
            if (this.state === STATE.INIT) {
                ...
                if (errorPacket) {
                    this.recievedData([...this.buffer.split(/\n/g), ...lines]);
                    this.buffer = "";
                } else {
                    this.buffer += msg;
                }
        ...
        if (VALID_EVENTS[event]) {
            for (let hook of Object.keys(callbacks)) {
                if (VALID_EVENTS[event][hook]) {
                    ...
                    this.registeredHooks[hook].push(callbacks[hook]);
                } else {
    ...
    registerEvent(event, options, callbacks) {
        let valid = this.registerHook(event, callbacks);
        if (valid) {
