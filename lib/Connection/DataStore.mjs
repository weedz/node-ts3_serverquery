...
export default class DataStore {
    ...
    fetchList(list, noCache = false) {
        return new Promise( async (resolve, reject) => {
            if (this.store[list] && this.store[list].fetching) {
                if (!this.store[list].queue) {
                    this.store[list].queue = [];
                }
                this.store[list].queue.push({resolve,reject});
            } else if (this.checkCacheAge(this.store[list], CACHE_EXPIRE[list], noCache)) {
                this.store[list] = {
                    fetching: true
                };
                try {
                    ...
                    resolve(data);
                    if (this.store[list].queue) {
                        for (let item of this.store[list].queue) {
                            item.resolve(data);
                        }
                    }
                } catch(err) {
    ...
    fetchInfo(list, keyName, keyValue, noCache = false) {
        ...
        return new Promise( async (resolve, reject) => {
            if (this.store[list][keyValue] && this.store[list][keyValue].fetching) {
                if (!this.store[list][keyValue].queue) {
                    this.store[list][keyValue].queue = [];
                }
                this.store[list][keyValue].queue.push({resolve,reject});
            } else if (this.checkCacheAge(this.store[list][keyValue], CACHE_EXPIRE[list], noCache)) {
                ...
                try {
                    ...
                    resolve(data);
                    if (this.store[list][keyValue].queue) {
                        for (let item of this.store[list][keyValue].queue) {
                            item.resolve(data);
                        }
                    }
                } catch(err) {
